...
class PillarRatingCalculator:
    ...
    def _valuation(self, raw: dict) -> Optional[float]:
        ...
        score_pe = _decile_score(pe, PE_CUTOFFS, lower_better=True) if pe is not None and pe > 0 else None
        score_pb = _decile_score(pb, PB_CUTOFFS, lower_better=True) if pb is not None and pb > 0 else None
        ...
        score_peg = _decile_score(peg, PEG_CUTOFFS, lower_better=True) if peg is not None else None

        return _weighted_avg({"pe": (score_pe, 0.4), "pb": (score_pb, 0.3), "peg": (score_peg, 0.3)})
    ...
    def _profitability(self, raw: dict) -> Optional[float]:
        ...
        score_m = _decile_score(margin_pct, MARGIN_CUTOFFS, lower_better=False) if margin_pct is not None else None
        score_r = _decile_score(roe_pct,    ROE_CUTOFFS,    lower_better=False) if roe_pct    is not None else None
    ...
    def _growth(self, raw: dict) -> Optional[float]:
        ...
        score_r = _decile_score(rev_pct, GROWTH_CUTOFFS, lower_better=False) if rev_pct is not None else None
        score_e = _decile_score(eps_pct, GROWTH_CUTOFFS, lower_better=False) if eps_pct is not None else None
    ...
    def _health(self, raw: dict) -> Optional[float]:
        ...
        score_de = _decile_score(de, DE_CUTOFFS, lower_better=True) if de is not None and de >= 0 else None
        score_cr = _decile_score(cr, CR_CUTOFFS, lower_better=False) if cr is not None else None
    ...
    def _cashflow(self, raw: dict) -> Optional[float]:
        ...
        score_ef = _decile_score(ev_fcf, EVFCF_CUTOFFS, lower_better=True) if ev_fcf > 0 else None
    ...
    def _efficiency(self, raw: dict) -> Optional[float]:
        ...
        score_at = _decile_score(asset_turn, ASSET_TURN_CUTOFFS, lower_better=False) if asset_turn is not None else None
        score_it = _decile_score(inv_turn,   INV_TURN_CUTOFFS,   lower_better=False) if inv_turn   is not None else None
...
_PILLAR_NAMES = ["valuation", "profitability", "growth", "health", "cashflow", "efficiency"]
...
class PillarValidator:
    ...
    def _sanity_check(self, result: PillarResult, raw_metrics: dict, stock_id: int) -> None:
    ...
    def _check_valuation_dominance(self, result: PillarResult, raw: dict, stock_id: int) -> None:
    ...
    def _check_profitability_dominance(self, result: PillarResult, raw: dict, stock_id: int) -> None:
        if result.profitability_score is None:
            return
        margin_ok = _first(raw, ["netProfitMarginTTM", "netProfitMarginAnnual"]) is not None
    ...
    def _check_growth_dominance(self, result: PillarResult, raw: dict, stock_id: int) -> None:
    ...
    def _check_health_dominance(self, result: PillarResult, raw: dict, stock_id: int) -> None:
    ...
    def _sensitivity_analysis(self, result: PillarResult, stock_id: int) -> None:
        ...
        for pillar_name in available:
            for direction, factor in [("+20%", 1.0 + self.SENSITIVITY_SHIFT),
                ...
                if abs(delta) > self.SENSITIVITY_LOG_THRESHOLD:
                    logger.info(
                        ...
                        "(%.4f → %.4f)",
                        stock_id, pillar_name, direction, delta, base, alt,
                    )
